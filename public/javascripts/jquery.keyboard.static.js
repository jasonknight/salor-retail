...
An on-screen virtual keyboardStatic embedded within the browser window which
...
Options:
	layout
		[String] specify which keyboardStatic layout to use
	...
	customLayout
		[Array] Specify a custom layout
			An Array of arrays.
			Each internal array is a new keyboardStatic row.
			...
			Special/"Action" keys include:
				{a}, {accept} - Updates element value and closes keyboardStatic
				{alt},{altgr} - AltGr for International keyboardStatic
				{b}, {bksp}   - Backspace
				{c}, {cancel} - Clears changes and closes keyboardStatic
...
CSS:
	.ui-keyboard-static { padding: .3em; position: absolute; left: 0; top: 0; z-index: 16000; }
	.ui-keyboard-static div { font-size: 1.1em; }
	.ui-keyboard-static-button { height: 2em; width: 2em; margin: .1em; }
	.ui-keyboard-static-widekey { width: 4em; }
	.ui-keyboard-static-space { width: 15em; }
	.ui-keyboard-static-preview { width: 100%; text-align: left; margin-bottom: 3px; }
	.ui-keyboard-static-keyset { text-align: center; }
	.ui-keyboard-static-input { text-align: left; }
	.ui-keyboard-static-overlay { height: 100%; width: 100%; background: transparent; position: absolute; top: 0; left: 0; z-index: 15999; }
*/

(function($){
	$.keyboardStatic = function(el, options){
		var base = this;
		...
		base.$el.data("keyboardStatic", base);

		base.init = function(){
			base.options = $.extend(true, {}, $.keyboardStatic.defaultOptions, options);
			...
			base.rows = ['ui-keyboard-static-keyset-default', 'ui-keyboard-static-keyset-shift', 'ui-keyboard-static-keyset-alt', 'ui-keyboard-static-keyset-alt-shift' ];
			base.acceptedKeys = [];
			base.mappedKeys = {};
			...
			// Display keyboardStatic on focus
			base.$el
				.addClass('ui-keyboard-static-input ui-widget-content ui-corner-all')
				.attr({ 'aria-haspopup' : 'true', 'role' : 'textbox' })
				.bind('focus.keyboardStatic', function(){
					base.focusOn();
				});
		...
		base.reveal = function(){
			// close all keyboardStatics
			//$('.ui-keyboard-static').hide();

			if (!base.options.usePreview) { base.$el.unbind('focus.keyboardStatic'); }

			// build keyboardStatic if it doesn't exist
			if (typeof(base.$keyboardStatic) === 'undefined') { base.startup(); }
			...
			// show & position keyboardStatic
			base.$keyboardStatic
				// position and show the keyboardStatic before positioning (required for UI position utility)
				.css({ position: 'absolute', left: '75%', top: 120, width: '280px' })
				.show();
/*
				.position({
					// get single target position || target stored in element data (multiple targets) || default, at the element
					of: base.options.position.of || base.$el.data('keyboardStaticPosition') || base.$el,
					my: base.options.position.my,
					at: base.options.position.at,
					collision: (base.options.usePreview) ? 'fit' : 'flip'
				})*/

			// adjust keyboardStatic preview window width - save width so IE won't keep expanding (fix issue #6)
			if (typeof base.width === 'undefined') { base.width = (base.options.usePreview) ? base.$keyboardStatic.width() : base.$el.width(); }
			base.$preview
				.css('width', base.width) // set preview width to fill keyboardStatic IE7 thinks 100% means across the screen
				.focus();
			base.isVisible = true;
			...
			if (base.msie){
				...
				// Add overlay under the keyboardStatic to prevent clicking in and not opening a new keyboardStatic while one is open
				$('<div class="ui-keyboard-static-overlay"></div>')
					.click(function(){
						$(this).remove();
						base.close();
					})
					.appendTo('body');
				if (!base.options.usePreview) { base.$el.addClass('ui-keyboard-static-overlay-input'); }
			}
		...
		base.startup = function(){
			base.$keyboardStatic = base.buildKeyboard();
			base.$allKeys = base.$keyboardStatic.find('.ui-keyboard-static-button');
			base.$preview = (base.options.usePreview) ? base.$keyboardStatic.find('.ui-keyboard-static-preview') : base.$el;
			base.preview = base.$preview[0];
			base.$decBtn = base.$keyboardStatic.find('.ui-keyboard-static-dec');
			...
			base.$preview
				...
				.bind('keyup', function(e){
					switch (e.which) {
						// Insert tab key
						case 9 :
							// Added a flag to prevent from tabbing into an input, keyboardStatic opening, then adding the tab to the keyboardStatic preview
							// area on keyup. Sadly it still happens if you don't release the tab key immediately because keydown event auto-repeats
							if (base.tab) {
								$.keyboardStatic.keyaction.tab(base);
								base.tab = false;
							}
							break;

						// Escape will hide the keyboardStatic
						case 27:
							base.close();
							break;
					}
				...
				.bind('keydown', function(e){
					...
					// Mapped Keys - allows typing on a regular keyboardStatic and the mapped key is entered
			...
			base.$keyboardStatic.appendTo('body');
			...
			base.$allKeys
				.bind(base.options.keyBinding, function(e){
					...
					if ($.keyboardStatic.keyaction.hasOwnProperty(action)) {
						$.keyboardStatic.keyaction[action](base,this);
					} else if (typeof key.action !== 'undefined') {
						txt = (base.wheel && !$(this).is('.ui-keyboard-static-actionkey')) ? key.curTxt : key.action;
						base.insertText(txt);
					}
	...
	base.showKeySet = function(el){
		var key, toShow;
		base.$keyboardStatic.find('.ui-keyboard-static-actionkey[name*=key_meta]').removeClass('ui-state-active');
		if (base.metaActive) {
			key = el.name.split('_')[1];
			base.$keyboardStatic
				.find('.ui-keyboard-static-alt, .ui-keyboard-static-shift, .ui-keyboard-static-actionkey[class*=meta]').removeClass('ui-state-active').end()
				.find('.ui-keyboard-static-actionkey.ui-keyboard-static-' + key).addClass('ui-state-active').end()
				.find('.ui-keyboard-static-keyset').hide().end()
				.find('.ui-keyboard-static-keyset-' + key ).show();
		} else {
			toShow = (base.shiftActive) ? 1 : 0;
			toShow += (base.altActive) ? 2 : 0;
			base.$keyboardStatic
				.find('.ui-keyboard-static-alt')[(base.altActive) ? 'addClass' : 'removeClass']('ui-state-active').end()
				.find('.ui-keyboard-static-shift')[(base.shiftActive) ? 'addClass' : 'removeClass']('ui-state-active').end()
				.find('.ui-keyboard-static-keyset').hide().end()
	...
	base.checkDecimal = function(){
		if (/\./.test(base.$decBtn.closest('.ui-keyboard-static').find('.ui-keyboard-static-preview').val())) {
			base.$decBtn
	...
	base.getLayers = function(el){
		var key, keys;
		key = el.attr('name');
		keys = el.closest('.ui-keyboard-static').find('input[name=' + key + ']').map(function(){
			return this.value;
		}).get();
		return keys;
	};
	...
	// Close the keyboardStatic, if visible. Pass a status of true, if the content was accepted (for the event trigger).
	base.close = function(accepted){
		if (base.$keyboardStatic.is(':visible')) {
			...
			base.$el
				...
				.removeClass('ui-keyboard-static-overlay-input') // added for IE overlay
				.blur();
			if (!base.options.usePreview) { base.$el.blur().bind('focus.keyboardStatic', function(){ base.focusOn(); }); }
			//base.$keyboardStatic.hide();
	...
	base.escClose = function(e){
		if ( e.target === base.el ) { return; } // necessary to prevent keyboardStatic closing when usePreview is false
		if ( !$(e.target).closest('.ui-keyboard-static').length ) {
	...
	base.keyBtn = $('<input />')
		...
		.addClass('ui-keyboard-static-button ui-state-default ui-corner-all');
	...
	base.addKey = function(keyName, name, regKey ){
		...
		// Action keys will have the 'ui-keyboard-static-actionkey' class
		// '\u2190'.length = 1 because the unicode is converted, so if more than one character, add the wide class
		keyType = (n.length > 1) ? ' ui-keyboard-static-widekey' : '';
		keyType += (regKey !== true) ? ' ui-keyboard-static-actionkey' : '';
		return base.keyBtn
			.clone()
			.attr({ 'name': 'key_' + keyName, 'title' : t })
			.data('key', { action: keyName, original: n, curTxt : n, curNum: 0 })
			.val( n )
			// add "ui-keyboard-static-" + keyName, if this is an action key (e.g. "Bksp" will have 'ui-keyboard-static-bskp' class)
			// add "ui-keyboard-static-" + unicode of 1st character (e.g. "~" is a regular key, class = 'ui-keyboard-static-126' (126 is the unicode value - same as typing &#126;)
			.addClass('ui-keyboard-static-' + ((regKey === true) ? keyName.charCodeAt(0) : keyName) + keyType );
	};
	...
	base.buildKeyboard = function(){
		...
		container = $('<div />')
			.addClass('ui-keyboard-static ui-widget-content ui-widget ui-corner-all ui-helper-clearfix')
			.attr({ 'role': 'textbox' })
			.hide();
		...
		if (base.options.usePreview) {
			base.$preview = base.$el.clone(false)
				...
				.removeClass('placeholder')
				.addClass('ui-widget-content ui-keyboard-static-preview ui-corner-all')
				.show(); // for hidden inputs
		} else {
			// No preview display, use element and reposition the keyboardStatic under it.
		...
		// setup custom keyboardStatic
		if (base.options.layout === 'custom') {
			$.keyboardStatic.layouts.custom = base.options.customLayout || { 'default' : ['{cancel}'] };
		}

		// Main keyboardStatic building loop
		$.each($.keyboardStatic.layouts[base.options.layout], function(set, keySet){
			if (set !== "") {
				sets++;
				newSet = $('<div />')
					.attr('name', set) // added for typing extension
					.addClass('ui-keyboard-static-keyset ui-keyboard-static-keyset-' + set)
					.appendTo(container)[(set === 'default') ? 'show' : 'hide']();

				for ( row = 0; row < keySet.length; row++ ){
					newRow = $('<div />')
						.addClass('ui-keyboard-static-row ui-keyboard-static-row' + row )
						.appendTo(newSet);
	...
	base.destroy = function() {
		...
		base.$keyboardStatic.remove();
		base.$el
			.removeClass('ui-keyboard-static-input ui-widget-content ui-corner-all')
			.removeAttr('aria-haspopup')
			.removeAttr('role')
			.unbind('focus accepted canceled hidden visible')
			.removeData('keyboardStatic');
	};
	...
	// Default keyboardStatic layouts
	$.keyboardStatic.layouts = {
		'alpha' : {
			'default': [
	...
	$.keyboardStatic.defaultOptions = {
		...
		position     : {
			...
			my : 'center top',
			at : 'center top',
			at2: 'center bottom' // used when "usePreview" is false (centers the keyboardStatic at the bottom of the input/textarea)
		},
		// preview added above keyboardStatic if true, original input/textarea used if false
		usePreview   : true,

		// *** change keyboardStatic language & look ***
		display : {
			...
			'space'  : 'Space:Space',
			't'      : '\u21e5:Tab',          // right arrow to bar (used since this virtual keyboardStatic works with one directional tabs)
		...
		// *** Useability ***
		// Auto-accept content when clicking outside the keyboardStatic (popup will close)
		autoAccept   : false,

		// Prevents direct input in the preview window when true
		lockInput    : false,

		// Prevent keys not in the displayed keyboardStatic from being typed in
		restrictInput: false,
		...
		// When the character is added to the input (clicking on the keyboardStatic)
	...
	$.fn.keyboardStatic = function(options){
		return this.each(function(){
			(new $.keyboardStatic(this, options));
		});
	};

	// This function breaks the chain, but returns
	// the keyboardStatic if it has been attached to the object.
	$.fn.getkeyboardStatic = function(){
		return this.data("keyboardStatic");
	};
