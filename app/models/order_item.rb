...
class OrderItem < ActiveRecord::Base
  ...
  def toggle_refund(x)
    ...
    if self.refunded then
      ...
      $User.get_meta.vendor.open_cash_drawer unless $Register.salor_printer or self.order.refunded # open cash drawer only if not called from the Order.toggle_refund function
    end
  end
