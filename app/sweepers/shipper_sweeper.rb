...
class ShipperSweeper < ActionController::Caching::Sweeper
  observe Shipper

  def after_create(shipper)
    expire_cache_for(shipper)
  end

  def after_update(shipper)
    expire_cache_for(shipper)
  end

  def after_destroy(shipper)
    expire_cache_for(shipper)
  end

private
  def expire_cache_for(shipper)
