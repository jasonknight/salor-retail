...
class ItemTypeSweeper < ActionController::Caching::Sweeper
  observe ItemType

  def after_create(item_type)
    expire_cache_for(item_type)
  end

  def after_update(item_type)
    expire_cache_for(item_type)
  end

  def after_destroy(item_type)
    expire_cache_for(item_type)
  end

private
  def expire_cache_for(item_type)
